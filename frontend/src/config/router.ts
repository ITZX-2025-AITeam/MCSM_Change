import { createRouter, createWebHashHistory, type RouteRecordRaw } from "vue-router"; // 移除了未使用的 Router 类型
...
/**
 * 动态添加卡片（路由）
 * @param card { path: string; name: string; permission?: number }
 */
export function addCardRoute(card: { path: string; name: string; permission?: number }) {
  router.addRoute({
    path: '/test',
    name: "test",
    component: LayoutContainer,
    meta: {
      mainMenu: true,
      permission: card.permission ?? ROLE.ADMIN
    }
  });
}
...
/**
 * 动态删除卡片（路由）
 * @param cardName 路由的 name
 */
export function removeCardRoute(cardName: string) {
  router.removeRoute(cardName);
}
// 结束
