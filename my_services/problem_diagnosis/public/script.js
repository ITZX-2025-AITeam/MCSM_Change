...
class DiagnosisSystem {
    ...
    async handleRemoteUpdate(data) {
        ...
        this.addNotification({
            ...
            message: `${data.filename} 的 ${sectionText} 内容已提交`
        });
    ...
    toggleCompareMode() {
        ...
        if (this.compareMode) {
            ...
            this.rightReport = null;
            this.renderComparePane('left', this.leftReport);
            this.renderComparePane('right', this.rightReport);
        } else {
    ...
    async renderComparePane(side, filename) {
        const paneContent = document.getElementById(side === 'left' ? 'leftPaneContent' : 'rightPaneContent');
        if (!filename) {
            // 列出可选报告
            const listHtml = this.reports.map(r => `
                <button class="compare-select-item" data-filename="${r.filename}">
            ...
            paneContent.querySelectorAll('.compare-select-item').forEach(btn => {
                btn.addEventListener('click', async () => {
                    ...
                    await this.assignCompareSide(side, target);
                });
            });
            return;
        }
        ...
        try {
            ...
            if (!resp.ok) throw new Error('load report failed');
    ...
    async assignCompareSide(side, filename) {
        if (side === 'left') {
            this.leftReport = filename;
        } else {
            this.rightReport = filename;
        }
        await this.renderComparePane(side, filename);
        // 若从空白进入，更新当前报告标题辅助
        const title = document.getElementById('reportTitle');
        title.textContent = '报告对比模式';
    }

    async handleCompareSelection(filename) {
        // 优先填充空位：左 -> 右 -> 覆盖左
        if (!this.leftReport) {
            await this.assignCompareSide('left', filename);
        } else if (!this.rightReport) {
            await this.assignCompareSide('right', filename);
        } else {
            await this.assignCompareSide('left', filename);
        }
    }
    ...
    addNotification({ filename, section, message }) {
        ...
        if (resp.ok) {
            ...
            if (Array.isArray(list)) {
                list.slice().reverse().forEach(n => {
                    app.addNotification({ filename: n.filename, section: n.section, message: n.message });
                });
            }
        }
    } catch (e) {
